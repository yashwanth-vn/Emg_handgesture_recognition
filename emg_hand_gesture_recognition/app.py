...
from src.emg_sources import CSVSource, SimulatedSource, HardwareSource
...
_hardware_stream_active = False
_hardware_stream_lock = threading.Lock()
...
# -----------------------------------------------------------------------------
# Hardware-specific endpoints and streaming
# -----------------------------------------------------------------------------


@app.route('/api/hardware/connect', methods=['POST'])
def hw_connect():
    """Connect to the hardware EMG sensor."""
    try:
        success = hardware_source.connect_sensor()
        return jsonify({'connected': success, 'status': hardware_source.get_sensor_status()})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def hw_disconnect():
    """Disconnect the hardware EMG sensor."""
    try:
        hardware_source.disconnect_sensor()
        return jsonify({'disconnected': True, 'status': hardware_source.get_sensor_status()})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/hardware/stream/start', methods=['POST'])
def hw_start_stream():
    """Start streaming from hardware sensor."""
    global _hardware_stream_active

    with _hardware_stream_lock:
        if _hardware_stream_active:
            return jsonify({'message': 'Hardware stream already active'}), 200

        success = hardware_source.start_stream()
        if not success:
            return jsonify({'success': False, 'message': 'Failed to start hardware stream'}), 500

        _hardware_stream_active = True

    return jsonify({'success': True, 'message': 'Hardware stream started'})


@app.route('/api/hardware/stream/stop', methods=['POST'])
def hw_stop_stream():
    """Stop streaming from hardware sensor."""
    global _hardware_stream_active

    with _hardware_stream_lock:
        hardware_source.stop_stream()
        _hardware_stream_active = False

    return jsonify({'success': True, 'message': 'Hardware stream stopped'})


@app.route('/api/hardware/stream/data')
def hardware_stream_data():
    """SSE endpoint streaming live hardware EMG samples and predictions."""
    def generate():
        predictor = get_predictor()
        ...
        while _hardware_stream_active:
            try:
                sample = hardware_source.get_sample()

                if sample is not None:
                    result = predictor.predict(sample)
                    result['emg_data'] = sample.tolist()
                    result['sensor_status'] = hardware_source.get_sensor_status()
                    yield f"data: {json.dumps(result)}\n\n"
        ...
        yield f"data: {json.dumps({'status': 'hardware_stream_ended'})}\n\n"
